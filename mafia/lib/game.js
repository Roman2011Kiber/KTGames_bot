// =====================================================================
//  Стан гри, фази, AI ботів, резолв ночі та голосування.
// =====================================================================

import { BOT_NAMES, AVATARS, pickRandom } from "./names.js";
import {
  ROLES, ROLE_LABEL, ROLE_ICON, ROLE_DESC,
  buildRoleDeck, roleTeam, mafiaCountFor,
} from "./roles.js";

export { ROLES, ROLE_LABEL, ROLE_ICON, ROLE_DESC, mafiaCountFor };
...
export function createSoloGame(opts) {
  ...
  const players = [
    {
      ...
      isHuman: true, role: roles[0], alive: true,
      avatar: humanAvatar,
    },
    ...botNames.map((name, i) => ({
      id: newId(), name,
      isHuman: false, isBot: true,
      role: roles[i + 1], alive: true,
      avatar: avatars[i] || "🎭",
    })),
  ];
  ...
  return {
    ...
    winner: null, mode: "solo",
  };
}

export function alivePlayers(g) { return g.players.filter((p) => p.alive); }
export function aliveByTeam(g, team) {
  return alivePlayers(g).filter((p) => roleTeam(p.role) === team);
}
export function findById(g, id) { return g.players.find((p) => p.id === id); }
...
export function startOnlineGame(lobby) {
  ...
  return {
    ...lobby,
    phase: "role-reveal", day: 0, players,
    night: {}, votes: {}, actions: {}, ready: {},
    log: [{
      day: 0, kind: "info",
      text: `Гру розпочато з ${total} гравцями. Ніч наближається...`,
    }],
    winner: null, started: true,
  };
}
