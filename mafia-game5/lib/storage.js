...
async function fsTx(code, fn) {
  ...
  await runTransaction(db, async tx => {
    ...
    const next = { ...(adv || room), lastActivityAt: Date.now() };
    tx.set(ref, next);
    result = next;
  });
  return result;
...
export async function createRoom(name, opts = {}) {
  ...
  const room = {
    ...
    phaseStartedAt: 0, sheriffShotDay: null,
    createdAt: Date.now(), lastActivityAt: Date.now(),
    visibility: opts.visibility || 'public',
    password:   opts.visibility === 'protected' ? (opts.password || '') : '',
  };
...
export async function joinRoom(code, name, opts = {}) {
  ...
  await runTransaction(db, async tx => {
    ...
    if (room.visibility === 'protected' && room.password) {
      if ((opts.password || '') !== room.password) throw new Error('Невірний пароль');
    }
    ...
    if (humansBefore >= 20) throw new Error('Кімната заповнена (20 гравців)');
    ...
    const newHumans    = humansBefore + 1;
    const maxBots      = Math.max(0, 20 - newHumans);
    const newBotCount  = Math.min(room.botCount || 0, maxBots);
    tx.set(ref, { ...room, players: newPlayers, botCount: newBotCount, lastActivityAt: Date.now() });
  });
...
export async function updateSettings(code, playerId, settings) {
  ...
  await fsTx(code, room => {
    ...
    return {
      ...
      ...(botCount     != null ? { botCount: Math.min(Math.max(0, 20 - room.players.filter(p => !p.isBot).length), Math.max(0, Number(botCount))) } : {}),
    };
  });
  return { ok: true };
}
...
export async function hostTick(code, myId) {
  ...
  await runTransaction(db, async tx => {
    ...
    // Auto-cancel rooms with 6h+ inactivity
    const lastAct = room.lastActivityAt || room.createdAt || 0;
    if (Date.now() - lastAct > INACTIVE_TTL_MS && room.phase !== 'cancelled' && room.phase !== 'result') {
      tx.set(ref, { ...room, phase: 'cancelled', lastActivityAt: Date.now() });
      return;
    }

    const adv = fsTryAdvance(room);
    if (adv) tx.set(ref, { ...adv, lastActivityAt: Date.now() });
  });
}

/**
 * List public and protected rooms that are in the lobby phase.
 * Used for the rooms discovery page.
 */
export async function listPublicRooms() {
  if (!USE_FIRESTORE) return [];
  // Let errors propagate — caller (rooms page) decides how to show them
  ...
  return snap.docs
    .map(d => d.data())
    .filter(r => !r.visibility || r.visibility === 'public' || r.visibility === 'protected')
    .filter(r => (r.lastActivityAt || r.createdAt || 0) >= cutoff)
    .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
}

/**
 * Leave a room. In lobby: removes player, transfers host if needed.
 * In other phases: no-op (player just navigates away).
 */
export async function leaveRoom(code, playerId) {
  ...
  await runTransaction(db, async tx => {
    ...
    if (!snap.exists()) return;
    const room = snap.data();
    if (room.phase !== 'lobby') return; // during game just navigate away

    const remaining      = room.players.filter(p => p.id !== playerId);
    const remainingHumans = remaining.filter(p => !p.isBot);

    if (room.hostId === playerId) {
      if (remainingHumans.length > 0) {
        tx.set(ref, { ...room, players: remaining, hostId: remainingHumans[0].id, lastActivityAt: Date.now() });
      } else {
        tx.set(ref, { ...room, phase: 'cancelled', lastActivityAt: Date.now() });
      }
    } else {
      tx.set(ref, { ...room, players: remaining, lastActivityAt: Date.now() });
    }
  });
}

/**
 * Transfer host rights to another human player in the room.
 * Can be called by the current host without leaving.
 */
export async function transferHost(code, currentHostId, newHostId) {
  if (!USE_FIRESTORE) throw new Error('Тільки у Firestore-режимі');

  await fsTx(code, room => {
    if (room.hostId !== currentHostId) throw new Error('Тільки поточний хост може передати права');
    const newHost = room.players.find(p => p.id === newHostId && !p.isBot);
    if (!newHost) throw new Error('Гравця не знайдено');
    return { ...room, hostId: newHostId };
  });
  return { ok: true };
}

/**
 * Permanently delete a room from Firestore (moderator action).
 */
