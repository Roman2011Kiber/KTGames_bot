...
import {
  ...
  leaveRoom, transferHost, deleteRoom,
} from '../lib/storage.js';
...
function renderProfileStats(stats) {
  ...
  return h('div', {}, [
    h('div', { style: 'display:grid;grid-template-columns:1fr 1fr;gap:8px' }, [
      statBox(t('room.stats.games'), stats.gamesPlayed || 0),
      statBox(t('room.stats.wins'),  stats.wins || 0),
      statBox(t('room.stats.kills'), kills),
      statBox('🎯 Win rate', wr + '%'),
    ]),
    (stats.mafiaKills > 0 || stats.sheriffKills > 0) && h('div', { style: 'margin-top:8px;font-size:.78rem;color:var(--muted)' }, [
      stats.mafiaKills  > 0 && h('span', { style: 'margin-right:12px' }, t('room.stats.mafiaKills',   { n: stats.mafiaKills })),
      stats.sheriffKills > 0 && h('span', {}, t('room.stats.sheriffKills', { n: stats.sheriffKills })),
    ]),
  ]);
}
...
export function OnlineRoomPage(container, { code }) {
  ...
  function onSurrender() {
    ...
    const others  = inLobby
      ? (room.players || []).filter(p => !p.isBot && p.id !== me?.id)
      : [];
    const hostNote = isHost && inLobby && others.length > 0
      ? t('room.leave.hostTransfer', { name: others[0]?.name || '' })
      : (isHost && inLobby ? t('room.leave.hostDelete') : t('room.leave.desc'));

    openModal(h('div', {}, [
      h('div.font-display.text-xl.gold-text.mb-3', {}, t('room.leave.title')),
      h('p.muted.mb-4', {}, hostNote),
      h('div', { style: 'display:flex;gap:10px' }, [
        h('button.btn.btn-ghost-gold', { onclick: closeModal, style: 'flex:1' }, t('room.leave.stay')),
        h('button.btn.btn-blood', {
          style: 'flex:1',
          onclick: async () => {
            closeModal();
            destroyChat();
            if (me) leaveRoom(code, me.id).catch(() => {});
            navigate('/');
          },
        }, t('room.leave.leave')),
      ]),
  ...
  function openProfile(player) {
    ...
    const canTransferHost = isHostMe && !isMe && !player.isBot;
    ...
    const roleSelector = canChangeRole && h('div', { style: 'margin-top:16px' }, [
      h('div', { style: 'font-size:.75rem;color:var(--muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:.05em' }, t('room.mod.role')),
      h('select.input', {
        ...
        onchange: async e => {
          ...
          try {
            ...
            notify(t('room.mod.changed', { name: player.name, role: ROLE_LABEL[newRole] || newRole }), 'success');
            closeModal();
    ...
    openModal(h('div', {}, [
      ...
      h('div.text-center', { style: 'margin-bottom:16px' }, [
        ...
        player.isBot && h('div.muted.text-sm', { style: 'margin-top:4px' }, t('room.profile.bot')),
      ...
      // Stats area
      h('div', { id: profId }, [
        h('p.muted.text-center.text-sm', {}, player.isBot ? '' : t('room.profile.loading')),
      ]),

      // Role change (mod only, non-lobby)
      roleSelector,

      // Transfer host (current host can transfer to any other human, in any phase)
      canTransferHost && h('button.btn.btn-ghost-gold', {
        style: 'margin-top:16px',
        onclick: async () => {
          haptic('medium');
          try {
            await transferHost(code, me.id, player.id);
            closeModal();
            notify(t('room.transferHost.notify', { name: player.name }), 'success');
          } catch (e) { notify(e.message, 'error'); }
        },
      }, t('room.transferHost.btn')),
      ...
      canKick && h('button.btn.btn-blood', {
        ...
        onclick: async () => {
          ...
          try {
            ...
            closeModal();
            notify(t('room.kick.notify', { name: player.name }), 'success');
          } catch (e) { notify(e.message, 'error'); }
        },
      }, t('room.kick.btn')),

      // Moderator: delete entire room
      isMod && !isMe && h('button.btn.btn-blood', {
        style: 'margin-top:10px;opacity:.7',
        onclick: async () => {
          if (!window.confirm(t('room.deleteRoom.confirm', { code }))) return;
          haptic('heavy');
          try {
    ...
    if (!player.isBot) {
      getPlayerStats(player.id).then(stats => {
        ...
        el.innerHTML = '';
        if (!stats) {
          el.appendChild(h('p.muted.text-center.text-sm', {}, t('room.profile.empty')));
          return;
        }
  ...
  function onRoomUpdate(raw) {
    ...
    if (!me && raw.myPlayerId) {
      me = { id: raw.myPlayerId, name: tgUser?.name || loadNickname() || t('room.defaultName'), avatar: '🎭' };
      saveMe(code, me);
    }
  ...
  function showGone() {
    destroyChat();
    mount(container, Shell({ title: t('room.title'), back: '/', children: [
      h('div.card.text-center', {}, [
        h('div', { style: 'font-size:4rem;margin-bottom:16px' }, '🚪'),
        h('div.font-display.text-2xl.gold-text', {}, t('room.closed.title')),
        h('p.muted.mt-3', {}, t('room.closed.desc')),
        h('a.btn.btn-ghost-gold', { href: '#/', style: 'display:inline-block;width:auto;padding:10px 24px;margin-top:20px' }, t('room.closed.back')),
      ]),
    ] }));
  }
  ...
  function renderLobby() {
    ...
    const botLink = `https://t.me/Ktgames_bot?start=${code}`;

    function shareRoom() {
      haptic('light');
      const tgwa = window.Telegram?.WebApp;
      const shareUrl = `https://t.me/share/url?url=${encodeURIComponent(botLink)}&text=${encodeURIComponent(t('room.shareText', { code }))}`;
      if (tgwa?.openTelegramLink) {
        tgwa.openTelegramLink(shareUrl);
      } else if (navigator.share) {
        navigator.share({ title: t('room.shareTitle', { code }), text: `${t('room.roomCode')}: ${code}`, url: botLink }).catch(() => {});
      } else {
        navigator.clipboard?.writeText(botLink).catch(() => {});
        notify(t('room.linkCopied'), 'success');
      }
    }
    ...
    function openSettings() {
      ...
      const timeOpts = [[30, t('room.settings.30s')],[60, t('room.settings.1m')],[150, t('room.settings.2m30s')],[300, t('room.settings.5m')]];
      openModal(h('div', {}, [
        h('div.font-display.text-xl.gold-text.mb-4', {}, t('room.settings.title')),
        h('label.label', {}, t('room.settings.day')),
        h('select.input', { onchange: e => (s.dayDur = Number(e.target.value)) },
          timeOpts.map(([v, l]) => h('option', { value: v, selected: v === s.dayDur }, l))),
        h('label.label.mt-3', {}, t('room.settings.night')),
        h('select.input', { onchange: e => (s.nightDur = Number(e.target.value)) },
          timeOpts.map(([v, l]) => h('option', { value: v, selected: v === s.nightDur }, l))),
        h('label.label.mt-3', {}, t('room.settings.bots')),
        h('select.input', { onchange: e => (s.bots = Number(e.target.value)) },
          Array.from({ length: Math.max(0, 20 - humans.length) + 1 }, (_, n) =>
            h('option', { value: n, selected: n === s.bots },
              n === 0 ? t('room.settings.noBots') : `${n}`))),
        h('div', { style: 'display:flex;gap:10px;margin-top:16px' }, [
          h('button.btn.btn-ghost-gold', { onclick: closeModal, style: 'flex:1' }, t('room.settings.cancel')),
          h('button.btn.btn-blood', {
            style: 'flex:1',
            onclick: async () => {
              try {
                await updateSettings(code, me.id, { dayDuration: s.dayDur, nightDuration: s.nightDur, botCount: s.bots });
                notify(t('room.settings.saved'), 'success');
                closeModal();
              } catch (e) { notify(e.message, 'error'); }
            },
    ...
    mount(container, Shell({ title: null, back: '/', children: [
      Card([
        h('div.text-center', {}, [
          h('p.text-xs.uppercase.tracking-mega.muted.mb-2', {}, t('room.roomCode')),
          h('div.font-display.text-5xl.gold-text.mb-3', {}, code),
          h('div.row', { style: 'justify-content:center;gap:8px;flex-wrap:wrap' }, [
            h('button.btn.btn-blood', { onclick: shareRoom, style: 'width:auto;padding:8px 18px;font-size:.85rem' }, t('room.share')),
            h('button.btn.btn-ghost-gold', { onclick: copyBotLink, style: 'width:auto;padding:8px 14px;font-size:.85rem' }, t('room.copy')),
          ]),
          h('div', { style: 'font-size:.7rem;color:var(--muted);margin-top:8px;word-break:break-all;font-family:monospace' }, botLink),
        ]),
      ], 'mb-4'),
      Card([
    ...
    mount(container, Shell({ title: null, back: null, children: [
      Card([
        h('div.text-center', {}, [
          ...
          allies.length > 0 && h('p.text-sm.accent.mb-3', {}, t('room.allies', { names: allies.map(p => p.name).join(', ') })),
          !revealDone && myAlive
            ? h('button.btn.btn-blood.mt-3', { onclick: onReveal }, t('room.confirmRole'))
            : h('p.muted.mt-3.text-sm', {}, t('room.waiting')),
        ]),
      ], 'mb-4'),
  ...
  function renderGame() {
    ...
    if (phase === 'night' && myAlive) {
      if (isMafia) {
            ...
            h('button.player-tile', {
              ...
              onclick: async () => {
                ...
                try {
                  ...
                  notify(t('room.target.notify', { name: c.name }), 'success');
        ...
        nightPanel = Card([
          h('div.font-display.text-lg.accent.mb-3', {}, t('room.heal.title')),
          h('p.muted.text-sm.mb-3', {}, t('room.heal.desc')),
          h('div.players-grid', {}, candidates.map(c =>
            h('button.player-tile', {
              ...
              onclick: async () => {
                ...
                try {
                  ...
                  notify(t('room.heal.notify', { name: c.name }), 'success');
          ...
          !healLocked && h('button.btn.btn-ghost-gold.mt-3', {
            onclick: async () => {
              if (healLocked) return;
              healLocked = true;
              try { await nightSkip(code, me.id); notify(t('room.heal.skipped'), 'info'); }
              catch (e) { notify(e.message, 'error'); healLocked = false; }
              render();
            },
          }, t('room.heal.skip')),
        ...
        nightPanel = Card([
          h('div.font-display.text-lg.accent.mb-2', {}, t('room.sheriff.title')),
          inv && h('div.card', { style: 'background:rgba(255,210,80,.07);margin-bottom:12px;padding:10px 14px;border-radius:10px' }, [
            h('p.text-sm.mb-1', {}, t('room.sheriff.result', { name: inv.targetName })),
            h('p.font-display', { style: `color:${inv.isMafia ? 'var(--blood-lit)' : 'var(--green,#4caf50)'}` },
              inv.isMafia ? '🔴 ' + t('role.mafia') + '!' : '🟢 ' + t('role.civilian')),
          ]),
          !shootLocked && h('div', {}, [
            h('p.muted.text-sm.mb-2', {}, t('room.sheriff.invDesc')),
            h('div.players-grid', {}, candidates.map(c =>
              h('button.player-tile', {
                onclick: async () => {
                  ...
                  try {
                    ...
                    notify(t('room.sheriff.invNotify', { name: c.name }), 'success');
                  } catch (e) { notify(e.message, 'error'); shootLocked = false; }
                  render();
                },
              }, [h('div.avatar', {}, c.avatar), h('div.name', {}, c.name)])
            )),
            h('p.muted.text-sm.mt-4.mb-2', {}, t('room.sheriff.shootDesc')),
            h('div.players-grid', {}, candidates.map(c =>
              h('button.player-tile', {
                ...
                onclick: async () => {
                  ...
                  try {
                    ...
                    notify(t('room.sheriff.shootNotify', { name: c.name }), 'success');
                  } catch (e) { notify(e.message, 'error'); shootLocked = false; }
                  render();
                },
              }, [h('div.avatar', {}, c.avatar), h('div.name', {}, c.name)])
            ...
            h('button.btn.btn-ghost-gold.mt-3', {
              onclick: async () => {
                if (shootLocked) return;
                shootLocked = true;
                try { await nightSkip(code, me.id); notify(t('room.sheriff.skipNotify'), 'info'); }
                catch (e) { notify(e.message, 'error'); shootLocked = false; }
                render();
              },
            }, t('room.sheriff.skipNight')),
          ]),
        ...
        votePanel = Card([
            ...
            h('button.player-tile', {
              onclick: async () => {
                ...
                try {
                  ...
                  notify(t('room.vote.notify', { name: c.name }), 'success');
          ...
          h('button.btn.btn-ghost-gold.mt-3', {
            onclick: async () => {
              if (voteLocked) return;
              voteLocked = true;
              try { await castVote(code, me.id, null); notify(t('room.abstain.notify'), 'info'); }
              catch (e) { notify(e.message, 'error'); voteLocked = false; }
              render();
            },
          }, UI.game.abstain),
    ...
    mount(container, Shell({ title: null, back: null, surrender: onSurrender, children: [
      h('div.row.space-between.mb-4', {}, [
        ...
        h('div.text-right', {}, [
          h('p.text-xs.muted', {}, t('room.codeLabel', { code })),
          h('p.text-xs.muted', {}, t('room.aliveLabel', { n: aliveList.length })),
  ...
  function renderResult() {
    ...
    // Record personal stats once — only for verified Telegram users
    if (!statsRecorded && me && !me.isBot && tgUser?.id) {
      statsRecorded = true;
      const myRole  = room.myRole;
      ...
      const myDoctorSaves  = klog.filter(k => k.healer === me.id && k.kind === 'doctorSave').length;
      recordMyStats(me.id, {
        nickname: me.name,
    ...
    mount(container, Shell({ title: t('room.final'), back: '/', children: [
  ...
  function showJoinModal() {
    let inputEl;
    openModal(h('div', {}, [
      h('div.font-display.text-xl.gold-text.mb-4', {}, t('room.join.title', { code })),
      h('label.label', {}, t('room.join.nameLabel')),
      h('input.input', { ref: el => (inputEl = el), placeholder: t('room.join.placeholder'), maxlength: 32 }),
      h('button.btn.btn-blood.mt-4', {
        onclick: async () => {
          const n = inputEl?.value?.trim();
          if (!n) { notify(t('room.join.errNoName'), 'error'); return; }
          haptic('medium');
          try {
            ...
            notify(e.message || t('room.join.errFailed'), 'error');
          }
        },
      }, t('room.join.btn')),
    ]), { dismissable: false });
