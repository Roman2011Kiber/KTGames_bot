...
function fsResolveNight(room) {
  ...
  for (const a of Object.values(actions)) { if (a?.action === 'heal' && a.target) healed.add(a.target); }
  ...
  const next = { ...room, players, log, investigations: invs, nightActions: {}, lastKilled, winner: w || null, phaseStartedAt: Date.now() };
  if (w) {
    next.phase = 'result';
...
function fsResolveVotes(room) {
  ...
  if (!bestId || tie) {
    ...
    if (l) { l.alive = false; log = fsAddLog(log, { day: room.day, kind: 'vote', text: `🪦 ${l.avatar} ${l.name} вигнаний. Він був ${ROLE_LABEL[l.role]?.toLowerCase() || l.role}.` }); }
  }

  const w = fsCheckWinner(players);
  const newDay = w ? room.day : room.day + 1;
  const next = { ...room, players, log, votes: {}, nightActions: {}, winner: w || null, day: newDay, phaseStartedAt: Date.now() };
  if (w) {
    next.phase = 'result';
    next.log = fsAddLog(next.log, { day: next.day, kind: 'win', text: w === 'town' ? '🌅 Місто перемогло!' : '🩸 Мафія захопила місто...' });
  } else {
...
function fsBuildResponse(room, playerId) {
  if (!room) return null;
...
export async function createRoom(name, opts = {}) {
  ...
  const room = {
    ...
    players: [{ id: pid, name: name.trim(), avatar: '🎭', role: 'civilian', alive: true, isBot: false }],
...
export async function joinRoom(code, name) {
  ...
  await runTransaction(db, async tx => {
    ...
    const existing = room.players.find(p => p.name.toLowerCase() === name.trim().toLowerCase() && !p.isBot);
    if (existing) { playerId = existing.id; return; }
    if (room.players.filter(p => !p.isBot).length >= 15) throw new Error('Кімната заповнена');
    playerId = fsNewId();
    tx.set(ref, { ...room, players: [...room.players, { id: playerId, name: name.trim(), avatar: '🎭', role: 'civilian', alive: true, isBot: false }] });
  });
...
export function subscribeRoom(code, playerId, cb) {
  ...
  (async () => {
    ...
    unsub = onSnapshot(ref, snap => {
      ...
      cb(fsBuildResponse(snap.data(), playerId));
    }, () => cb(null));
  })();
  return () => unsub();
}
